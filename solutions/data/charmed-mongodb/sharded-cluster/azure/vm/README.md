...
| `juju_space.peers` | Peer spaces in the config-server and shard models |
| `juju_subnet.peers` | Existing Azure subnet assignments for the peer spaces |
...
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- | :--------: |
| `network_spaces` | CIDR of the existing Azure subnet for the `peers` spaces | `object({ peers_cidr = optional(string, "10.3.0.0/24") })` | `{}` | no |
...
in the config-server model and every shard model. It assigns an existing Azure
subnet to these spaces before deploying the MongoDB applications.
The subnet must already exist in the VNet referenced by the infrastructure state.
The default matches the `clouds/azure` peer subnet:
...
Override this CIDR to match your infrastructure. It must be a valid IPv4 CIDR.
The space name remains fixed.

| Application | Endpoints on `peers` |
| ----------- | -------------------- |
| Config server | `database-peers`, `config-server`, `cluster` |
| Each shard | `database-peers`, `sharding` |
| Mongos | `router-peers`, `cluster` |

Config-server, shard, and Data Integrator machines use
`arch=amd64 spaces=peers instance-type=Standard_D8_v3` by default.
Retain `spaces=peers` when customizing their constraints.
Existing machines supplied through `machines` must have
