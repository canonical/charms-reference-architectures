# Terraform module for MongoDB Sharded Cluster on Azure Kubernetes Service

This Terraform root module facilitates the deployment of a Charmed MongoDB
sharded cluster on Azure Kubernetes Service (AKS) using the
...
The AKS cluster must already be registered as a Juju cloud. By
...
| `s3_integrator`                         | Optional S3-compatible backup-integrator configuration                                                                                                                                            | <pre>object({<br/>  config      = map(string)<br/>  channel     = optional(string, "2/stable")<br/>  base        = optional(string, "ubuntu@24.04")<br/>  revision    = optional(number, null)<br/>  constraints = optional(string, "arch=amd64")<br/>  machines    = optional(set(string), [])<br/>})</pre>                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `null`                                                                           | no         |
...
- `region`: region expected by the selected object-storage service.
- `endpoint`: S3-compatible API endpoint exposed by the service.
...
![Charmed MongoDB sharded cluster deployment](docs/sharded-cluster-azure-k8s.excalidraw.svg)

Authenticate with Azure and obtain the AKS kubeconfig if the cluster has not
already been registered with Juju:
